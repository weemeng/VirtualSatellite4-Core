 ...
 *
 */
public class RemoveReqIFNatureHandler extends AbstractHandler implements IHandler {
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selection instanceof TreeSelection) {
			...
			if (select.getFirstElement() instanceof IProject) {
				...
				try {
					removeProjectNature(project, ReqIFNature.NATURE_ID);
				} catch (CoreException e) {
					Activator.getDefault().getLog().log(new Status(Status.ERROR, Activator.getPluginId(), "Could not remove the ReqIF nature form the project", e));
				}
			}
		}
		return null;
