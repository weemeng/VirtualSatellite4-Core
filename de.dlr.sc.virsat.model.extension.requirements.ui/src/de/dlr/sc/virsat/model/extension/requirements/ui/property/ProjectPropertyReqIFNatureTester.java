 ...
 *******************************************************************************/
package de.dlr.sc.virsat.model.extension.requirements.ui.property;
 ...
 *
 */
public class ProjectPropertyReqIFNatureTester extends PropertyTester {
	...
	public static final boolean hasNature(final IProject project) {
		try {
			IProjectNature nature = project.getNature(ReqIFNature.NATURE_ID);
			return !(nature == null);

		} catch (final CoreException e) {
			Activator.getDefault().getLog().log(new Status(Status.ERROR, Activator.getPluginId(), "Could not check if ReqIF nature is enabled", e));
		}
		return false;
	}
